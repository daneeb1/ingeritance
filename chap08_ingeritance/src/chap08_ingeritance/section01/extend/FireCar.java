...
public class FireCar extends Car {
	/* car 클래스를 부모클래스로 FireCar를 지식클래스로 사용할 것이다.
	 * 클래스 선언부에 extends Car를 추구해서 Car 클래스를 상속 받을 것이다.
	 * 그러면 fireCar는 부모클래스인 car가 가지고 있는 모든 멤버를 상속받는다.
	 * 하지만 생성자는 상속받지 못한다.
	 * 또한 부모가 가지고 있는 private 멤버라는 접근이 불가능하다.
	 */
	...
	public FireCar() {
		 ...
		 */
		super();
		System.out.println("FireCar 클래스의 기본 생성자가 호출");
	}
	...
	@Override
	public void soundHorn() {	//소방차에서만 적용됨.
		/* 여기서 부모가 가지고 있는 멤버인 isRunning() 메소드를 사용해야
		 * 주행중인 상태인지를 확인할 수 있는데 확인할 수 있는 방법이 없다.
		 * car클래스의 isRunnung 메소드 접근제한자를 protected로 변경하고 확인해 본다.
		 */
